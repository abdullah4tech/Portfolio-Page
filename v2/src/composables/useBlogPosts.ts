...
function ensureParsed() {
  ...
  Object.entries(modules).forEach(([path, raw]) => {
    ...
    const fileSlug = path.match(/\/([^/]+)\.md$/)?.[1]?.replace(/^\d{4}-\d{2}-\d{2}-/, '') || ''

    _posts.value.push({
      slug: data.slug || fileSlug,
      title: data.title || 'Untitled',
      date: data.date || '',
      author: data.author || 'Abdullah O. Mustapha',
...
export function useBlogPosts() {
  ...
  const getPostBySlug = (slug: string) => _posts.value.find((p) => p.slug === slug)

  const getPostsByTag = (tag: string) => posts.value.filter((p) => p.tags?.includes(tag))
